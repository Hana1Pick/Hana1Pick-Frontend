      ...
      <div className="content">
        <div>
          <div className="withdrawMsg">
            <div>
              <h3 className="narrowLine">모아클럽과 연결될 계좌를<br />
                선택해 주세요.</h3>
            </div>
            <div>
              <h4>연결 계좌는 변경할 수 없으니<br />
                신중하게 선택해 주세요.</h4>
            </div>
          </div>
        </div>
        <div className="withdraw-box1">
          ...
          <div className="withdraw-box2">
            <label htmlFor="lang" style={{ fontSize: "12px" }}>연결계좌</label> &nbsp;
            <select name="languages" id="lang" style={{ border: "0", width: "70%", backgroundColor: "#F8F8F9" }} onChange={handleSelectChange}>
              <option value="" disabled selected>계좌를 선택해 주세요.</option>
              {account && (
                <option value={account.accountId}>{account.name}의 입출금 계좌 ({getAccCode(account.accountId)})</option>
              )}
            </select>
          </div>
        </div>

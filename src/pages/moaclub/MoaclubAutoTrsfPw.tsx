...
function MoaclubAutoTrsfPw() {
	...
	const outAccId = req.selectedAccount;
  ...
  */
	...
	const next = () => {
		const url = `http://${process.env.REACT_APP_BESERVERURI}/api/moaclub/auto-transfer/setting`;
		...
		axios
			...
			.then((res) => {
				if (res.data.status === 201) {
					navigate('/moaclub/autotrsf/complete', {
						state: { outAccId, inAccId, moaclub },
					});
				}
			})
			.catch((error) => {
...
export default MoaclubAutoTrsfPw;
